dir = getDirectory("Directory path for saving");
dir_comp = getDirectory("Path to composite images");
...
thresholds = newArray(0,1.4,1.5,1.6,1.7,1.8,1.9,2,2.1,2.2,2.3,2.4,2.5, 4); 
...
for (i=0; i<list.length; i++) {
	...
	open(dir_comp + "composite"+substring(list[i], 9, lengthOf(list[i])));
	run("8-bit");
	...
	close("SUM_composite" + substring(list[i], 9, lengthOf(list[i])));
