...
for (i=0; i<list.length; i++) {
	...
	run("Gaussian Blur...", "sigma=3");
	//run("Duplicate...", " ");
	run("Merge Channels...", "c1=[SUM_C4-"+list[i]+"] c2=[SUM_C2-"+list[i]+"] c3=[SUM_C3-"+list[i]+"] create keep");
...
//roiManager("Select", 0);
	...
	selectWindow("Result of SUM_C2-"+list[i]);
	rename("SUM_C2C3-"+list[i]);
	imageCalculator("Divide create 32-bit", "SUM_C2-"+list[i],"SUM_C4-"+list[i]);
	...
	imageCalculator("Add create", "SUM_C2-"+list[i],"SUM_C4-"+list[i]);
	rename("SUM_C2C4-"+list[i]);
	imageCalculator("Divide create 32-bit", "SUM_C2-"+list[i],"SUM_C2C4-"+list[i]);
	rename("Percentage_C2_of_C2C4-"+list[i]);
	saveAs("Tiff", "C:/Users/zuzka/Desktop/rasy/processed/"+"Percentage_C2_of_C2C4-"+list[i]+".tif");
	imageCalculator("Add create", "SUM_C3-"+list[i],"SUM_C2C4-"+list[i]);
	rename("SUM_C2C3C4-"+list[i]);
	imageCalculator("Divide create 32-bit", "SUM_C2-"+list[i],"SUM_C2C3C4-"+list[i]);
